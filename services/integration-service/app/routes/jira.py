"""
Jira integration routes for Integration Service.

Handles Jira API interactions for:
...
def get_jira_config() -> JiraConfig | None:
    """Get Jira configuration from database/secrets."""
    # In a full implementation, this would read from a secrets store
...
@router.get("/config")
async def get_jira_configuration() -> dict[str, Any]:
...
@router.post("/config")
async def update_jira_configuration(config: JiraConfig) -> dict[str, str]:
...
async def create_jira_issue_from_finding(
    ...
    # Get the finding details from database
    engine = get_engine()
    ...
    with engine.connect() as conn:
        ...
        finding = result.mappings().first()
    
    if not finding:
        raise HTTPException(status_code=404, detail="Finding not found")
    
    # Build issue summary and description
    summary = f"[Code Review] {finding.get('rule_name') or 'Code Quality Issue'}"
    
    description_parts = [
        f"**Finding Details:**",
        f"- Rule: {finding.get('rule_name') or 'Unknown'}",
        f"- Severity: {finding.get('severity')}",
        f"- File: {finding.get('file_path')}",
        f"- Line: {finding.get('line_start') or 'N/A'}",
        "",
        f"**Description:**",
        finding.get('message') or "No description available",
        "",
    ]
    ...
    description_parts.append(f"**Analysis ID:** {finding.get('analysis_id')}")
...
async def link_jira_issue_to_finding(
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text("SELECT id FROM findings WHERE id = :finding_id"),
            {"finding_id": request.finding_id}
        )
        finding = result.mappings().first()
...
async def get_finding_jira_issue(finding_id: str) -> JiraIssue | None:
    """Get linked Jira issue for a finding."""
    config = get_jira_config()
    if not config:
        return None
    
    # In a full implementation, look up the linked issue key
    # For now, return None
    return None
