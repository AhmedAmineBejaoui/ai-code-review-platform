...
export function IconsaxThemeModeButton({ className }: { className?: string }) {
      ...
      <span className={cn("inline-flex h-6 w-6 items-center justify-center rounded-full", isDark ? "bg-slate-800" : "bg-slate-100")}>
        <AnimatePresence mode="wait" initial={false}>
          {mounted && (
            ...
            >
              {/* Utilise les icônes Iconsax au lieu de Lucide */}
              {isDark ? (
                <div className="h-4 w-4 flex items-center justify-center">
                  <Icon name="flash" variant="Bold" color="rgb(251, 191, 36)" size="sm" />
                </div>
              ) : (
                <div className="h-4 w-4 flex items-center justify-center">
                  <Icon name="flash" variant="Linear" color="rgb(99, 102, 241)" size="sm" />
                </div>
              )}
            </motion.span>
          )}
