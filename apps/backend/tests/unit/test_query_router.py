...
def test_query_router_detects_document_query() -> None:
    router = QueryRouter()
    assert router.route_query(query="Summarize this PDF architecture guide") == QueryRoute.DOCUMENT_QUERY


def test_query_router_detects_repo_query_from_path() -> None:
    router = QueryRouter()
    assert router.route_query(query="Explain apps/backend/app/main.py") == QueryRoute.REPO_QUERY
